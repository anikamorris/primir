...
extension UITableView {
    func reloadWithAnimation() {
        ...
        var delayCounter = 0
        for cell in cells {
            cell.transform = CGAffineTransform(translationX: 0, y: tableViewHeight * 0.5)
        }
        for cell in cells {
            UIView.animate(withDuration: 0.8, delay: 0.1 * Double(delayCounter),usingSpringWithDamping: 0.5, initialSpringVelocity: 0, options: .curveEaseInOut, animations: {
